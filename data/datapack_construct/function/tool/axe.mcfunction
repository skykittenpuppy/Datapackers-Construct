...
$item replace $(where) with wooden_axe

$item modify $(where) datapack_construct:axe

$item modify $(where) { \
"function": "set_components", \
"components": { \
	"custom_data": {"tool_type": "axe", "materials": [$(head), $(binding), $(handle)]}, \
	"custom_model_data": {strings: ["$(head)", "$(binding)", "$(handle)"]}, \
	"item_model": "datapack_construct:axe", \
	"max_stack_size": 1, \
	"!food": {} \
}}
